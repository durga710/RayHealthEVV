...
import { z } from 'zod';
import { AuditEventRepository, EvvRepository, ClientRepository, CaregiverRepository, AgencyHhaexchangeConfigRepository, AgencySandataConfigRepository, SandataClient, SandataAltEvv, HhaexchangeClient, buildHhaexchangeExport, toHhaexchangeCsv, } from '@rayhealth/core';
import { requireCapability } from '../middleware/require-capability.js';
import { safeError } from '../security/safe-log.js';
const router = Router();
const isYmdOrIso = (v) => !v || /^\d{4}-\d{2}-\d{2}(T.*)?$/.test(v);
...
router.post('/sandata/reconcile', requireCapability('billing.write'), async (req, res) => {
    ...
    try {
        ...
        for (const r of parsed.data.results) {
            const ok = await repo.markSandataSubmission(r.visitId, req.auth.agencyId, r.status, r.confirmationId ?? undefined);
            if (ok)
                updated += 1;
            else
                notFound.push(r.visitId);
        }
        try {
            await new AuditEventRepository(db).create({
                agencyId: req.auth.agencyId,
    ...
    try {
        ...
        if (!config) {
            return res.status(422).json({
                message: 'HHAeXchange is not fully configured for this agency. Set the Tax ID, Provider ID, caregiver and service-code mappings before exporting.',
            });
        }
        let q = db('evv_visits as v')
            .join('users as u', 'u.caregiver_id', 'v.caregiver_id')
            ...
            .leftJoin('clients as c', 'c.id', db.raw('coalesce(v.client_id, t.client_id)'))
            .where('u.agency_id', req.auth.agencyId)
            .select('v.id as visit_id', 'v.caregiver_id', 'c.id as client_id', 'c.external_id as client_external_id', 'c.first_name as client_first_name', 'c.last_name as client_last_name', 'v.service_code', 'v.clock_in_time', 'v.clock_out_time', 'v.clock_in_location', 'v.clock_out_location')
            .orderBy('v.clock_in_time', 'asc');
        if (fromIso)
            q = q.andWhere('v.clock_in_time', '>=', fromIso);
        if (toIso)
        ...
        const visits = rows.map((r) => {
            ...
            return {
                ...
                memberId: r.client_external_id ?? r.client_id ?? '',
                ...
                internalServiceCode: r.service_code ?? '',
...
router.post('/hhaexchange/reconcile', requireCapability('billing.write'), async (req, res) => {
    ...
    try {
        ...
        const notFound = [];
        for (const r of parsed.data.results) {
            const ok = await repo.markHhaexchangeSubmission(r.visitId, req.auth.agencyId, r.status, r.confirmationId ?? undefined);
            if (ok)
                updated += 1;
            else
                notFound.push(r.visitId);
...
function buildAltEvvConfig(cfg) {
    if (!cfg)
        return { ok: false, reason: 'Sandata integration has not been set up for this agency' };
    if (!cfg.enabled)
        return { ok: false, reason: 'Sandata integration is disabled for this agency' };
    if (!cfg.apiBaseUrl)
        return { ok: false, reason: 'No Sandata API base URL configured' };
...
router.post('/sandata/altevv/submit', requireCapability('billing.write'), async (req, res) => {
    ...
    try {
        ...
        if (!built.ok)
            return res.status(409).json({ status: 'not_configured', reason: built.reason });
        ...
        const rows = (await new EvvRepository(db).getVisitsForExport(agencyId, fromIso, toIso));
        ...
        const clientIds = new Set(verifiedRows.map((r) => r.clientId).filter((id) => Boolean(id)));
        const caregiverIds = new Set(verifiedRows.map((r) => r.caregiverId).filter(Boolean));
        const allClients = (await new ClientRepository(db).getClients(agencyId));
        const allCaregivers = (await new CaregiverRepository(db).findByAgency(agencyId));
