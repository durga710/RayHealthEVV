export class LearningRepository {
     ...
     * returns undefined.
     */
    async updateCourse(id, agencyId, data) {
        ...
        if (data.expiresAfterDays !== undefined)
            patch.expires_after_days = data.expiresAfterDays;
        if (data.required !== undefined)
            patch.required = data.required;
        if (data.durationMinutes !== undefined)
            patch.duration_minutes = data.durationMinutes;
        if (data.externalUrl !== undefined)
            patch.external_url = data.externalUrl ?? null;
        if (data.modules !== undefined)
            patch.modules = data.modules ? JSON.stringify(data.modules) : null;
        const [row] = await this.db('learning_courses')
     ...
     */
    async deleteCourse(id, agencyId) {
     ...
     */
    async findEnrollment(caregiverId, courseId, agencyId) {
     ...
     */
    async markInProgress(enrollmentId, agencyId) {
    ...
    // ---------- Completions ----------
    async recordCompletion(data, agencyId) {
     ...
     */
    async getCertificate(courseId, caregiverId) {
        const row = await this.db('course_enrollments as e')
            ...
            .first('e.id as enrollment_id', 'e.last_completed_at', 'e.expires_at', 'cg.first_name', 'cg.last_name', 'c.title as course_title', 'c.code as course_code', 'c.cadence', 'a.name as agency_name');
        if (!row)
            return undefined;
        const r = row;
